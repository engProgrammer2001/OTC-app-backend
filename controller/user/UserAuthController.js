...
class UserAuthController {
    ...
    static RegisterUser = async (req, resp) => {
        try {
            const { first_name, last_name, email, password, confirm_password, referal_code } = req.body;
            ...
            let referalData;
            if (referal_code) {
                referalData = await User.findOne({ user_code: referal_code });
                if (!referalData) {
                    return handleResponse(404, "Invalid referral code", {}, resp);
                }
            }
            ...
            const user = new User({
                ...
                otp: genotp,
                user_code: generateReferralCode(),
                refered_by: referalData?.id ?? null
            });
            ...
            const earning = new ReferalEarning({
                user_id: user?.id,
                earning: 0
            })
