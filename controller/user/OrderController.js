    ...
    static CreateOrder = async (req, resp) => {
        try {
            ...
            const order = new Order({
                ...
                total_amount: Number(quantity_purchased) * Number(planData?.price),
                order_id: orderId,
                status: "pending",
            })
            ...
            const validDate = moment(user?.createdAt).add(1, "month")
            const allUserOrders = await Order.find({ user_id: user.id })
            if (allUserOrders && Array.isArray(allUserOrders) && allUserOrders.length === 1 && currentDate.isBefore(validDate)) {
                const referedByData = await User.findOne({ id: user.refered_by })

                if (referedByData) {
                    const referalData = new Referals({
                        user_id: user?.id,
                        referal_id: referedByData?.id ?? null,
                        order_id: order?.id
                    })
